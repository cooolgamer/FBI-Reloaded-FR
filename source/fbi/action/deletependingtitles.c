...
static bool action_delete_pending_titles_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_pending_titles_draw_top, res, "Impossible d'effacer ce logiciel\n(Failed to delete pending title.)");
    return true;
}
...
static void action_delete_pending_titles_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("Succès", "Logiciel(s) en attente effacé(s).", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_pending_titles_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Effacement des logiciel(s) en attente", "Appuyez sur B pour annuler.", true, data, action_delete_pending_titles_update, action_delete_pending_titles_draw_top);
        } else {
...
static void action_delete_pending_titles_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            error_display_res(NULL, NULL, loadingData->popData.result, "Impossible d'obtenir la liste des logiciels.\n(Failed to populate pending title list.)");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Récupération de la liste des logiciels...");
}
...
void action_delete_pending_title(linked_list* items, list_item* selected) {
    action_delete_pending_titles(items, selected, "Effacer ce logiciel en attente?", false);
}

void action_delete_all_pending_titles(linked_list* items, list_item* selected) {
    action_delete_pending_titles(items, selected, "Effacer tous les logiciels en attente?", true);
}