...
static void action_delete_system_save_data_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_FAILED(res)) {
        error_display_res(info, task_draw_system_save_data_info, res, "Impossible d'effacer cette sauvegarde système.\n(Failed to delete system save data.)");
    } else {
        linked_list_remove(deleteData->items, deleteData->selected);
        task_free_system_save_data(deleteData->selected);

        prompt_display_notify("Succès", "Sauvegarde système effacée.", COLOR_TEXT, NULL, NULL, NULL);
    }
...
static void action_delete_system_save_data_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Effacement de la sauvegarde", "", false, data, action_delete_system_save_data_update, action_delete_system_save_data_draw_top);
    } else {
        free(data);
    }
}
...
void action_delete_system_save_data(linked_list* items, list_item* selected) {
    ...
    prompt_display_yes_no("Confirmation", "Effacer cette sauvegarde système?", COLOR_TEXT, data, action_delete_system_save_data_draw_top, action_delete_system_save_data_onresponse);
}