...
static bool action_install_url_error(void* data, u32 index, Result res, ui_view** errorView) {
    ...
    if(strlen(url) > 38) {
        *errorView = error_display_res(data, action_install_url_draw_top, res, "Impossible d'installer depuis l'URL.\n%.35s...", url);
    } else {
        *errorView = error_display_res(data, action_install_url_draw_top, res, "Impossible d'installer depuis l'URL.\n%.38s", url);
    }
...
static void action_install_url_confirm_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Installation depuis internet", "Appuyez sur B pour annuler.", true, data, action_install_url_install_update, action_install_url_draw_top);
        } else {
