...
static void action_delete_ext_save_data_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_FAILED(res)) {
        error_display_res(info, task_draw_ext_save_data_info, res, "Impossible d'effacer la sauvegarde.\n(Failed to delete ext save data.)");
    } else {
...
static void action_delete_ext_save_data_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Effacement de la sauvegarde", "", false, data, action_delete_ext_save_data_update, action_delete_ext_save_data_draw_top);
    } else {
        free(data);
    }
}
