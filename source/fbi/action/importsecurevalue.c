...
static void action_import_secure_value_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_SUCCEEDED(res)) {
        prompt_display_notify("Succès", "Secure value importée.", COLOR_TEXT, info, task_draw_title_info, NULL);
    } else {
        error_display_res(info, task_draw_title_info, res, "Impossible d'importer la secure value.\n(Failed to import secure value.)");
    }
}

static void action_import_secure_value_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Importation de la secure value", "", false, data, action_import_secure_value_update, task_draw_title_info);
    }
}

void action_import_secure_value(linked_list* items, list_item* selected) {
    prompt_display_yes_no("Confirmation", "Importer la secure value de ce logiciel?", COLOR_TEXT, selected->data, task_draw_title_info, action_import_secure_value_onresponse);
}