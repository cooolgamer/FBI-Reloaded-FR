...
static void action_extract_smdh_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_SUCCEEDED(res)) {
        prompt_display_notify("Succès", "SMDH extrait.", COLOR_TEXT, info, task_draw_title_info, NULL);
    } else {
        error_display_res(info, task_draw_title_info, res, "Impossible d'extraire le SMDH.\n(Failed to extract SMDH.)");
    }
}

static void action_extract_smdh_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Extraction du SMDH", "", false, data, action_extract_smdh_update, task_draw_title_info);
    }
}
