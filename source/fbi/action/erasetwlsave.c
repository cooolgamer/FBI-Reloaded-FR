...
static bool action_erase_twl_save_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(((erase_twl_save_data*) data)->title, task_draw_title_info, res, "Impossible d'effacer la sauvegarde\n(Failed to erase save.)");
    return true;
}
...
static void action_erase_twl_save_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(eraseData->eraseInfo.finished) {
        ...
        if(R_SUCCEEDED(eraseData->eraseInfo.result)) {
            prompt_display_notify("Succès", "Sauvegarde effacée.", COLOR_TEXT, eraseData->title, task_draw_title_info, NULL);
        }
...
static void action_erase_twl_save_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Effacement de la sauvegarde", "Appuyez sur B pour annuler.", true, data, action_erase_twl_save_update, action_erase_twl_save_draw_top);
        } else {
